...
sealed abstract trait Trampoline[+Result] {
  import Trampoline._
...
object Trampoline {
  private final case class Done[+Result](a: Result) extends Trampoline[Result]
  private final case class Thunk[+Result](thunk: () => Trampoline[Result]) extends Trampoline[Result]

  def done[Result](r:Result): Trampoline[Result] = Done(r)
  def tailCall[Result](thunk: => Trampoline[Result]) = Thunk(() => thunk)
}
